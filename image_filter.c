...
static errno_t init_module_CLI()
{
    ...
    strcpy(data.cmd[data.NBcmd].Ccall,
           "long fconvolve(cconst har *ID_in, const char *ID_ke, const char *ID_out)");
    data.NBcmd++;
    ...
    for(jj = filter_size; jj < naxes[1] - filter_size; jj++)
        for(ii = filter_size; ii < naxes[0] - filter_size; ii++)
        {
            for(i = 0; i < (2 * filter_size + 1); i++)
                ...
                {
                    array[i * (2 * filter_size + 1) + j] = data.image[ID].array.F[(jj - filter_size
                                                           + j) * naxes[0] + (ii - filter_size + i)];
                }
            quick_sort_float(array, (2 * filter_size + 1) * (2 * filter_size + 1));
            data.image[ID_out].array.F[jj * naxes[0] + ii] = array[((
                        2 * filter_size + 1) * (2 * filter_size + 1) - 1) / 2];
        }
    free(array);
...
//
long FILTER_percentile_interpol(const char *ID_name, const char *IDout_name,
                                double perc, double sigma)
{
    ...
    for(k = 0; k < naxes[2]; k++)
    {
        ...
        for(jj = 0; jj < naxes[1]; jj++)
        {
            for(ii = 0; ii < naxes[0] - (2 * filtersizec + 1); ii++)
            {
                for(i = 0; i < (2 * filtersizec + 1); i++)
                {
                    data.image[ID_tmp].array.F[jj * naxes[0] + (ii + filtersizec)] += array[i] *
                            data.image[ID].array.F[k * naxes[0] * naxes[1] + jj * naxes[0] + (ii + i)];
                }
            }
            for(ii = 0; ii < filtersizec; ii++)
            {
                ...
                {
                    data.image[ID_tmp].array.F[jj * naxes[0] + ii] += array[i] *
                            data.image[ID].array.F[k * naxes[0] * naxes[1] + jj * naxes[0] +
                                                   (ii - filtersizec + i)];
                    tot += array[i];
                }
        ...
        for(ii = 0; ii < naxes[0]; ii++)
        {
            ...
            for(jj = 0; jj < naxes[1] - (2 * filtersizec + 1); jj++)
            {
                ...
                {
                    data.image[ID_out].array.F[k * naxes[0]*naxes[1] + (jj + filtersizec)*naxes[0] +
                                               ii] += array[j] * data.image[ID_tmp].array.F[(jj + j) * naxes[0] + ii];
                }
            }
            ...
            for(jj = 0; jj < filtersizec; jj++)
            {
                ...
                for(j = filtersizec - jj; j < jmax; j++)
                {
                    ...
                    fflush(stdout);
                    data.image[ID_out].array.F[k * naxes[0]*naxes[1] + jj * naxes[0] + ii] +=
                        array[j] * data.image[ID_tmp].array.F[(jj - filtersizec + j) * naxes[0] + ii];
                    tot += array[j];
                }
...
int gauss_3Dfilter(const char *ID_name, const char *out_name, float sigma,
                   int filter_size)
{
    ...
    for(kk = 0; kk < naxes[2]; kk++)
        for(jj = 0; jj < naxes[1]; jj++)
            for(ii = 0; ii < naxes[0] - (2 * filter_size + 1); ii++)
            {
                for(i = 0; i < (2 * filter_size + 1); i++)
                {
                    data.image[ID_tmp].array.F[kk * naxes[0]*naxes[1] + jj * naxes[0] +
    ...
    for(kk = 0; kk < naxes[2]; kk++)
        for(ii = 0; ii < naxes[0]; ii++)
            for(jj = 0; jj < naxes[1] - (2 * filter_size + 1); jj++)
            {
                for(j = 0; j < (2 * filter_size + 1); j++)
                {
                    data.image[ID_tmp1].array.F[kk * naxes[0]*naxes[1] + (jj + filter_size)*naxes[0]
                                                + ii] += array[j] * data.image[ID_tmp].array.F[kk * naxes[0] * naxes[1] +
                                                         (jj + j) * naxes[0] + ii];
                }
            }
    ...
    for(ii = 0; ii < naxes[0]; ii++)
        for(jj = 0; jj < naxes[1]; jj++)
            for(kk = 0; kk < naxes[2] - (2 * filter_size + 1); kk++)
            {
                ...
                {
                    data.image[ID_out].array.F[(kk + filter_size)*naxes[0]*naxes[1] + jj * naxes[0]
                                               + ii] += array[k] * data.image[ID_tmp1].array.F[(kk + k) * naxes[0] * naxes[1] +
                                                        jj * naxes[0] + ii];
                }
            }
...
long fconvolve(const char *name_in, const char *name_ke, const char *name_out)
{
    ...
    if((naxes[0] != data.image[ID_ke].md[0].size[0])
            || (naxes[1] != data.image[ID_ke].md[0].size[1]))
    {
        fprintf(stderr,
...
long fconvolve_padd(const char *name_in, const char *name_ke, long paddsize,
                    const char *name_out)
{
    ...
    for(ii = 0; ii < naxes[0]; ii++)
        for(jj = 0; jj < naxes[1]; jj++)
        {
            data.image[ID1].array.F[(jj + paddsize)*naxespadd[0] + (ii + paddsize)] =
                data.image[ID_in].array.F[jj * naxes[0] + ii];
            data.image[ID2].array.F[(jj + paddsize)*naxespadd[0] + (ii + paddsize)] =
                data.image[ID_ke].array.F[jj * naxes[0] + ii];
...
int fconvolveblock(const char *name_in, const char *name_ke,
                   const char *name_out, long blocksize)
{
        ...
        for(i = 0; i < NBfrequ1D; i++)
            for(j = 0; j < 2 * NBfrequ1D - 1; j++)
            {
                if(fscanf(fp, "%ld %ld %ld %g %g\n", &i, &j, &tmpl,
                          &coscoeff[j * NBfrequ1D + i], &sincoeff[j * NBfrequ1D + i]) != 5)
                {
                    printf("ERROR: fscanf, %s line %d\n", __FILE__, __LINE__);
                    exit(0);
                }
            ...
            for(j = 0; j < 2 * NBfrequ1D - 1; j++)
            {
                ...
                for(ii = 0; ii < size; ii += step)
                    for(jj = 0; jj < size; jj += step)
                    {
                        ...
                        if(rarray[ii1] < 1.0)
                        {
                            data.image[IDres].array.F[ii1] = data.image[ID].array.F[ii1] -
...
)
{
    ...
    for(ii = 0; ii < size; ii++)
        for(jj = 0; jj < size; jj++)
        {
            ...
            cosa = x / (r + 0.000001);
            //Cs = 3.34e-05;
            tmp1 = r * CX1_1 + r * r * CX2_1 + r * r * r * CX3_1 + r * r * r * r * CX4_1 + r
                   * r * r * r * r * CX5_1;
            tmp2 = r * CX1_2 + r * r * CX2_2 + r * r * r * CX3_2 + r * r * r * r * CX4_2 + r
                   * r * r * r * r * CX5_2;
            ...
            data.image[ID2].array.F[jj * size + ii] = data.image[ID1].array.F[jj * size +
                    ii];
            data.image[ID3].array.F[jj * size + ii] = data.image[ID].array.F[jj * size + ii]
                    ...
                    for(i = 0; i < NBgridpts1D; i++)
                        for(j = 0; j < NBgridpts1D; j++)
                        {
                            ...
                            if((fabs(x1) < 1.0) && (fabs(y1) < 1.0))
                            {
                                ...
                                Varraytmp[j * NBgridpts1D + i] += value * data.image[ID3].array.F[jj * size +
                                                                  ii];
        ...
        for(ii = 0; ii < size; ii++)
            for(jj = 0; jj < size; jj++)
            {
                ...
                if(r < 1.0)
                {
                    data.image[ID3].array.F[jj * size + ii] = data.image[ID].array.F[jj * size + ii]
                            - data.image[ID2].array.F[jj * size + ii];
                    error += data.image[ID3].array.F[jj * size + ii] * data.image[ID3].array.F[jj *
                             size + ii];
                    cnt ++;
                }
            }
...
long filter_CubePercentile(const char *IDcin_name, float perc,
                           const char *IDout_name)
{
    long IDcin;
    long IDout;
...
long filter_CubePercentileLimit(const char *IDcin_name, float perc, float limit,
                                const char *IDout_name)
{
    long IDcin;
    long IDout;
